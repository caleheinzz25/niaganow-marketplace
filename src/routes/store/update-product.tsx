...
import { clientQuery } from "~/router";
...
export const Route = createFileRoute("/store/update-product")({
...
function RouteComponent() {
  ...
  const queryProduct = useQuery(() => getProduct(context.search));
  ...
  const [uploadedImages, setUploadedImages] = createStore<ImageInfo[]>([]);
  const [thumbnail, setThumbnail] = createStore<ImageInfo>({
    filename: "",
    url: "",
  });
  ...
  createEffect(() => {
    if (queryProduct.isSuccess) {
      ...
      setThumbnail("url", queryProduct.data.thumbnail);
      setUploadedImages((images) =>
        product.images.map((url) => ({ url, filename: "" }))
      );
